    ...
    <div className="relative max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      <section className="text-center max-w-2xl mx-auto">
        ...
        <p className="text-zinc-650 text-sm sm:text-base mt-4 leading-relaxed">
          ARCOS products are trusted by a wide range of industries, manufacturing custom component solutions for large OEM brands, contractors, and national distribution grids.
        </p>
      </section>

      {/* 2. Industries Grid with Images */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {industries.map((ind, idx) => {
          ...
          return (
            <div 
              key={idx}
              className="bg-white border border-zinc-200 rounded-2xl overflow-hidden flex flex-col justify-between hover:border-primary/20 transition-all duration-300 relative group shadow-sm"
            >
              <div>
                {/* Visual Image Banner with overlay */}
                <div className="relative w-full h-[150px] overflow-hidden bg-zinc-50 border-b border-zinc-100">
                  <img
                    src={ind.image}
                    alt={ind.name}
                  ...
                  <div className="absolute top-4 left-4 h-9 w-9 rounded-lg bg-primary/90 text-white flex items-center justify-center border border-white/20">
                    <Icon className="h-4.5 w-4.5" />
                  </div>
                </div>

                <div className="p-5 space-y-2">
                  <h3 className="text-base font-bold text-zinc-950 tracking-tight group-hover:text-primary transition-colors">
                    {ind.name}
                  </h3>
      ...
      {/* 3. Core OEM Applications Section */}
      <section className="bg-zinc-900 text-white rounded-[30px] p-8 sm:p-12 shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
          <div className="lg:col-span-6 space-y-4">
            ...
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              From residential ceiling fans to high-capacity APFC distribution grids and outdoor solar systems, ARCOS electrical capacitors, PE cable clips, and self-locking Nylon ties maintain structural power stability.
            </p>
          </div>
            ...
            {applications.map((app, idx) => (
              ...
              >
                <div className="h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center border border-primary/30 text-primary shrink-0">
                  <span className="text-[10px] font-mono font-bold">{(idx + 1).toString()}</span>
                </div>
                <span className="text-zinc-200 text-xs font-semibold leading-none">{app}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white border border-zinc-200 rounded-[30px] p-8 sm:p-12 shadow-sm text-center max-w-3xl mx-auto space-y-6">
        ...
        <p className="text-zinc-600 text-xs sm:text-sm leading-relaxed">
          Are you a brand manufacturer or an electrical buyer looking for specialized microfarad ratings, dimensions, custom brackets, terminal styles, or private label labeling? Coordinate with our Noida design team to build custom capacitors for your motors, panels, or pumps.
        </p>
